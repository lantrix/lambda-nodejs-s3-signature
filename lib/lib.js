...
function signHeaders(headers) {
	...
	let thisHeaders = new Buffer(headers, 'utf-8'); //hmac doesn't want unicode
	let parts = thisHeaders.split('\n');
	...
	let signature = crypto.hmacSHA256(signed_key, (string_to_sign).encode('utf-8')).toString(crypto.enc.Hex);
